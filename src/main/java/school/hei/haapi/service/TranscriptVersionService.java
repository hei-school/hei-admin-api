...
public class TranscriptVersionService {
  ...
  public List<TranscriptVersion> getTranscriptsVersions(
      String studentId, String transcriptId, PageFromOne page, BoundedPageSize pageSize) {
    Pageable pageable =
        PageRequest.of(page.getValue() - 1, pageSize.getValue(), Sort.by(DESC, "creationDatetime"));
    return repository.findAllByTranscriptStudentIdAndTranscriptId(
        studentId, transcriptId, pageable);
  }

  public TranscriptVersion getTranscriptVersion(
      String studentId, String transcriptId, String versionId) {
    if (Objects.equals(versionId, "latest")) {
      return repository
          .findFirstByTranscriptStudentIdAndTranscriptIdOrderByRefDesc(studentId, transcriptId)
    ...
    return repository
        ...
        .orElseThrow(
            () -> new NotFoundException("TranscriptVersion with id " + versionId + " not found"));
  }

  public List<TranscriptVersion> getTranscriptVersionByStudentAndTranscriptId(
      String sId, String tId, PageFromOne page, BoundedPageSize pageSize) {
    Pageable pageable =
        PageRequest.of(page.getValue() - 1, pageSize.getValue(), Sort.by(DESC, "creationDatetime"));
    return repository.findAllByTranscriptStudentIdAndTranscriptId(sId, tId, pageable);
  }

  public byte[] getTranscriptVersionPdfByStudentIdAndTranscriptIdAndVersionId(
      String studentId, String transcriptId, String versionId) {
    ...
    if (key.isEmpty()) {
      throw new NotFoundException(
          "No pdf attached to the transcriptVersion with id: " + transcriptId);
    }
  ...
  @Transactional
  public TranscriptVersion addNewTranscriptVersion(
      String studentId, String transcriptId, String editorId, byte[] pdfFile) {
    Transcript transcript = transcriptService.getByIdAndStudent(transcriptId, studentId);
    if (transcript.getIsDefinitive()) {
      throw new BadRequestException(
          "Transcript with id " + transcriptId + " is already definitive");
    }
    User student = userService.getById(studentId);
    User editor = userService.getById(editorId);
    int newRef = 1;
    if (!getTranscriptVersionByStudentAndTranscriptId(
            studentId, transcriptId, new PageFromOne(1), new BoundedPageSize(10))
        .isEmpty()) {
