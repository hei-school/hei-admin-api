...
public class TranscriptClaimMapper {
  ...
  public StudentTranscriptClaim toRest(TranscriptClaim transcriptClaim) {
    return new StudentTranscriptClaim()
        ...
        .status(
            StudentTranscriptClaim.StatusEnum.fromValue(
                transcriptClaim.getClaimStatus().toString()));
  }
  ...
  public TranscriptClaim toDomain(
    ...
    if (claim != null && claim.isPresent()) {
      ...
      if (claim.get().getClaimStatus().equals(TranscriptClaim.ClaimStatus.OPEN)
          && Objects.equals(
              studentTranscriptClaim.getStatus(), StudentTranscriptClaim.StatusEnum.CLOSE)) {
    ...
    if (!Objects.equals(studentTranscriptClaim.getId(), claimId)) {
      throw new BadRequestException(
          "Id in request body "
              + studentTranscriptClaim.getId()
              + " is different from id in path variable "
              + claimId);
    }
    ...
    return TranscriptClaim.builder()
        ...
        .transcriptVersion(
            transcriptVersionService.getTranscriptVersion(studentId, transcriptId, versionId))
        .build();
  }
}
